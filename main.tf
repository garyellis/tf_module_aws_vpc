...
resource "aws_eip" "natgw_eip" {
  ...
  vpc  = true
  tags = merge(var.tags, map("Name", format("%s-natgw-%s", var.name, substr(element(var.azs, count.index), -1, 1))))
}
...
resource "aws_subnet" "public" {
  ...
  tags              = merge(var.tags, map("Name", format("%s-%s-%s", var.name, lookup(var.public_subnets[count.index], "name") , substr(element(var.azs, count.index), -1, 1))))
}
...
resource "aws_subnet" "private" {
  ...
  tags              = merge(var.tags, map("Name", format("%s-%s-%s", var.name, lookup(var.private_subnets[count.index], "name") , substr(element(var.azs, count.index), -1, 1))))
}
...
resource "aws_subnet" "private_restricted" {
  ...
  tags              = merge(var.tags, map("Name", format("%s-%s-%s", var.name, lookup(var.private_restricted_subnets[count.index], "name") , substr(element(var.azs, count.index), -1, 1))))
}
